#include "mlir/Conversion/DMAToLLVM/DMAToLLVM.h"
...
namespace mlir {
#define GEN_PASS_DEF_CONVERTDMATOLLVMPASS
...
struct WaitForTransferOpLowering : ConvertOpToLLVMPattern<WaitForTransferOp> {
  ...
  matchAndRewrite(WaitForTransferOp op, OpAdaptor adaptor,
    ...
    Value lastCompleted = rewriter.create<snitch_dma::StatOp>(op->getLoc());
    Value notDone =
...
struct StatOpLowering : ConvertOpToLLVMPattern<snitch_dma::StatOp> {
  using ConvertOpToLLVMPattern<snitch_dma::StatOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(snitch_dma::StatOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
...
void mlir::populateDMAToLLVMConversionPatterns(
  ...
  typeConverter.addConversion([](dma::TokenType token) {
...
namespace {
struct ConvertDMAToLLVMPass
    : public mlir::impl::ConvertDMAToLLVMPassBase<ConvertDMAToLLVMPass> {
  using Base::Base;
...
void ConvertDMAToLLVMPass::runOnOperation() {
