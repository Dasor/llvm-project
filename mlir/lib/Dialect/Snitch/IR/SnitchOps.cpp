...
TensorMicrokernelOp::getBufferType(Value value,
  ...
  if (auto memRefType = dyn_cast<BaseMemRefType>(corresponding.getType()))
    return cast<BufferLikeType>(memRefType);
...
PipelineOp::getBufferType(Value value,
  ...
  if (argument.getParentRegion()->getRegionNumber() != 0) {
    ...
    if (auto memRef = dyn_cast<BaseMemRefType>(yielded.getType()))
      return cast<BufferLikeType>(memRef);

    return bufferization::getBufferType(yielded, options, state,
                                        invocationStack);
  }

  FailureOr<BufferLikeType> initTypeOrFailure = bufferization::getBufferType(
      getTiedInit(argument).get(), options, state, invocationStack);
  if (failed(initTypeOrFailure))
    return failure();
  auto initType = cast<BaseMemRefType>(*initTypeOrFailure);
  ...
  if (!yieldedType) {
    FailureOr<BufferLikeType> maybeYieldedType = bufferization::getBufferType(
        yieldedValue, options, state, invocationStack);
    if (failed(maybeYieldedType))
      return failure();

    yieldedType = cast<BaseMemRefType>(*maybeYieldedType);
  }
    ...
    for (BlockArgument arg : oldRegion.getArguments()) {
      ...
      if (oldRegion.getRegionNumber() == 0) {
        ...
        replacements.push_back(rewriter.create<bufferization::ToTensorOp>(
            init.getLoc(), cast<TensorType>(arg.getType()), init));
        continue;
      }

      FailureOr<BufferLikeType> memRef =
          bufferization::getBufferType(arg, options, state);
      if (failed(memRef))
        return failure();

      BlockArgument newArg =
          newBlock.addArgument(cast<BaseMemRefType>(*memRef), arg.getLoc());
      replacements.push_back(rewriter.create<bufferization::ToTensorOp>(
          newArg.getLoc(), cast<TensorType>(arg.getType()), newArg));
    }
