...
LogicalResult StartTensorCopyOp::verify() {
  ...
  unsigned numDynamicPads =
      getStaticHighPad()
          ? llvm::count(*getStaticHighPad(), ShapedType::kDynamic)
          : 0;
...
///
/// Note: `getBufferType`/`getBuffer` now require a `BufferizationState`. Some
/// call sites of this helper (the `AnalysisState`-only interface methods)
/// don't have access to the "real" one used by the ongoing bufferization, so
/// a throwaway instance is used there. `BufferizationState` only caches a
/// symbol table, which is irrelevant for this op's memory-space-matching
/// query.
std::optional<bool> StartTensorCopyOp::elidesAllocation(
  ...
  bufferization::BufferizationState localState;
  const bufferization::BufferizationState &s = state ? *state : localState;

  FailureOr<BufferLikeType> copyType =
      invocationStack
          ? bufferization::getBufferType(getCopy(), options, s, *invocationStack)
          : bufferization::getBufferType(getCopy(), options, s);
  if (failed(copyType))
    return std::nullopt;

  FailureOr<BaseMemRefType> copyMemRefType = bufferization::detail::asMemRefType(copyType);
...
StartTensorCopyOp::getBufferType(Value value,
  ...
  if (!contained)
    if (elidesAllocation(options, &state, &invocationStack) == true)
...
StartTensorCopyOp::bufferize(RewriterBase &rewriter,
  ...
  std::optional<bool> result = elidesAllocation(options, &state);
  if (!result)
    return failure();
  ...
  FailureOr<BufferLikeType> allocBufferType =
      bufferization::getBufferType(getResult(), options, state);
  if (failed(allocBufferType))
    return failure();
  FailureOr<BaseMemRefType> allocType =
      bufferization::detail::asMemRefType(allocBufferType);
  if (failed(allocType))
    return failure();
  ...
  FailureOr<Value> alloc = options.allocationFn(
      rewriter, getLoc(), llvm::cast<MemRefType>(*allocType), dynamicDims,
