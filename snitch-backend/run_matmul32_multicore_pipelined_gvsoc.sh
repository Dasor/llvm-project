#!/usr/bin/env bash
# Matmul 32x32 in Snitch backend, multicore, pipelined, dual-buffered, run in gvsoc.
set -euo pipefail
...
[ -f "$SNRT/lib/libsnRuntime.a" ] || die "$SNRT/lib/libsnRuntime.a not found -- see runtime/snruntime/PROVENANCE.md"
command -v gvrun >/dev/null 2>&1 || die "gvrun not on PATH -- run ./build.sh first"
...
# Step 11: run the final executable in gvsoc
GVSOC_WORK_DIR="$OUT/out_multicore_pipelined_gvsoc"
mkdir -p "$GVSOC_WORK_DIR"

GVSOC_LOG="$GVSOC_WORK_DIR/gvsoc_run.log"
set +e
gvrun --target snitch --work-dir "$GVSOC_WORK_DIR" --param chip/soc/binary="$OUT/matmul32_multicore_pipelined" run \
  | tee "$GVSOC_LOG"
GVRUN_STATUS=$?
set -e
...
echo "[+] gvsoc run passed: dual-buffered pipelining + all 8 compute cores, combined"
